...
class AntMiniEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(
        self,
        xml_file="ant_mini_sensor_gyro.xml",
        forward_weight=5.0,
        ctrl_cost_weight=0.5,
        contact_cost_weight=5e-4,
        joint_force_weight=5e-4,
        touch_sensor_weight=5e-4,
        healthy_reward=1.0,
        ...
        self._joint_force_weight = joint_force_weight
        self._touch_sensor_weight = touch_sensor_weight
    ...
    def step(self, action):
        ...
        contact_cost = self.contact_cost(observation[7:(7+54)])
        joint_force_cost = self.joint_force_cost(observation[61:(61+8)])
        touch_cost = self.touch_cost(observation[-4:])
        #print(contact_cost)

        forward_reward = self._forward_weight * x_velocity - abs(y_velocity)
        healthy_reward = self.healthy_reward
        #print(self.pos[2])

        rewards = forward_reward #+ healthy_reward
        costs = ctrl_cost + contact_cost + joint_force_cost
        ...
        info = {
            ...
            "reward_joint_force": -joint_force_cost,
